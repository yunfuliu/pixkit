...
//////////////////////////////////////////////////////////////////////////
//	quality assessment (there still has some logics which  need to be modified)
//////////////////////////////////////////////////////////////////////////
void pixkit::qualityassessment::AutoCorrelation(cv::Mat &HI, std::vector< std::vector<double> > &Correlation, int BlockSize)
{
	//////////////////////////////////////////////////////////////////////////
	// exception
	if(HI.type()!=CV_8U){
		CV_Error(CV_BadNumChannels,"[qualityassessment::AutoCorrelation] accepts only grayscale image");
	}
	...
	for (int i=-BlockSize; i<=BlockSize; i++)
	{
		for (int j=-BlockSize; j<=BlockSize; j++)
		{
			SumValue = 0.0;
			PixelCounter = 0.0;
			for (int x=0; x<HI.rows; x++)
			{
				if (i+x<0 || i+x>=HI.rows)
					continue;
				for (int y=0; y<HI.cols; y++)
				{	
					if (j+y<0 || j+y>=HI.cols)
						continue;
					if (HI.data[x*HI.cols + y]==255 && HI.data[(i+x)*HI.cols + (j+y)]==255)
						SumValue++;
					PixelCounter = PixelCounter + 1;
				}
			}
			Correlation[i+BlockSize][j+BlockSize] = SumValue/PixelCounter;
		}
	}
